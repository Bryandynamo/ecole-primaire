...
<div class="container">
    ...
    <form action="{{ route('bulletins.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="eleve_id" class="form-control" required>
                ...
                @foreach($eleves as $eleve)
                    <option value="{{ $eleve->id }}" {{ old('eleve_id') == $eleve->id ? 'selected' : '' }}>{{ $eleve->nom }} {{ $eleve->prenom }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="trimestre" class="form-control" value="{{ old('trimestre') }}" required>
        </div>
        <div class="mb-3">
            <label for="moyenne" class="form-label">Moyenne</label>
            <input type="number" step="0.01" name="moyenne" class="form-control" value="{{ old('moyenne') }}" required>
        </div>
        <div class="mb-3">
            <label for="decision" class="form-label">Décision</label>
            <input type="text" name="decision" class="form-control" value="{{ old('decision') }}">
        </div>
