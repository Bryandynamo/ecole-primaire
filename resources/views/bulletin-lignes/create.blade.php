...
@section('content')
<div class="container">
    <h1>Ajouter une ligne de bulletin</h1>
    @if($errors->any())
    ...
    @endif
    <form action="{{ route('bulletin-lignes.store') }}" method="POST">
        @csrf
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select name="bulletin_id" class="form-control" required>
                ...
                @foreach($bulletins as $bulletin)
                    <option value="{{ $bulletin->id }}" {{ old('bulletin_id') == $bulletin->id ? 'selected' : '' }}>{{ $bulletin->id }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
            ...
            <select name="sous_competence_id" class="form-control" required>
                ...
                @foreach($sous_competences as $sc)
                    <option value="{{ $sc->id }}" {{ old('sous_competence_id') == $sc->id ? 'selected' : '' }}>{{ $sc->nom }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
            ...
            <select name="modalite_id" class="form-control" required>
                ...
                @foreach($modalites as $modalite)
                    <option value="{{ $modalite->id }}" {{ old('modalite_id') == $modalite->id ? 'selected' : '' }}>{{ $modalite->nom }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
            <label for="note" class="form-label">Note</label>
            <input type="number" step="0.01" name="note" class="form-control" value="{{ old('note') }}" required>
        </div>
