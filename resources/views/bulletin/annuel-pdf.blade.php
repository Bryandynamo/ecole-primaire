...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/bulletin_officiel.css') }}">
    <style>
        ...
        <div class="notes-section">
            ...
            <table class="table-bulletin" style="width:100%; border-collapse:collapse;">
                <tr>
                    ...
                    <th colspan="{{ count($uaNums)*2 }}" class="center border b">ANNUEL<br>Notes par UA</th>
                </tr>
                <tr>
                        ...
                        <td class="border small">{{ $mod['mod'] }}</td>
                                @foreach($uaNums as $ua)
                                    <td class="border center">
                                        {{ array_key_exists($ua, $b['notes'][$mod['scid']][$mod['modid']] ?? []) ? $b['notes'][$mod['scid']][$mod['modid']][$ua] : '-' }}
                                    </td>
                                    <td class="border center">
                                        {{ array_key_exists($ua, $b['notes'][$mod['scid']][$mod['modid']] ?? []) ? cote($b['notes'][$mod['scid']][$mod['modid']][$ua]) : '-' }}
                                    </td>
                                @endforeach
                    </tr>
                        @endforeach
            ...
            <table class="table-bulletin" style="width:100%; font-size:6pt; border-collapse:collapse;">
                <tr>
                    <td colspan="4" class="b" style="padding:0.5px 1px; background:#e1f7d5; text-align:center; font-size:7pt;">
                        ...
                        @else
                            Moyenne annuelle : <span style="font-weight:bold; font-size:8pt;">{{ $b['moyenne'] ?? '' }}</span> / 20 <span style="font-weight:bold; color:#007bff;">({{ rangOrdinal($b['rangTrimestre'] ?? 0) }})</span>
                        @endif
                    </td>
                </tr>
                <tr>
